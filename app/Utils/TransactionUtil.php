...
class TransactionUtil extends Util
{
     ...
     */
    public function createOrUpdateSellLinesOld($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
        ...
        foreach ($products as $product) {
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                ...
                $line = [
                    ...
                    'imei_id' => !empty($product['imeis']) ? json_encode($product['imeis']) : null,
                    'warranty' => !empty($product['warranty']) ? $product['warranty'] : null,
                ];
                ...
                if(!empty($product['imeis'])){
                    foreach($product['imeis'] as $imei){
                        $imei_details = ProductImei::findOrFail($imei);
                        $imei_details->sold_out=1;
                        $imei_details->save();
                    }
                }
        ...
        foreach ($products as $product) {
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                ...
                $line = [
                    ...
                    'secondary_unit_quantity' => ! empty($product['secondary_unit_quantity']) ? $this->num_uf($product['secondary_unit_quantity']) : 0,
                    'imei_id' => !empty($product['imeis']) ? json_encode($product['imeis']) : null,
                    'warranty' => !empty($product['duration']) ? $product['duration'] : null
                ];
