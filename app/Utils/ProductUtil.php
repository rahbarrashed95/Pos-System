...
class ProductUtil extends Util
{
     ...
     * @return array
     */
    public function getDetailsFromVariationOld($variation_id,$business_id, $location_id = null, $check_qty = true)
    {
        ...
        //         ->leftJoin('product_imeis as PI', function ($join) use ($imei_number) {
        //             $join->on('p.id', '=', 'PI.product_id')
        //                 ->where('PI.imei_number', $imei_number);
        //         })
     ...
     public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            if(!empty($data['imei'])){    
                foreach ($data['imei'] as $imei)
                {   
                    if($imei['imei_number'] != null){
                        if(!empty($imei['imei_id'])){
                            $imei_line=  ProductImei::find($imei['imei_id']);
                        }else{
                            $imei_line = new ProductImei;
                        }
                        $imei_line->product_id = $data['product_id'];
                        $imei_line->purchase_line_id= $purchase_line->id;
                        $imei_line->imei_number= $imei['imei_number'];
                        $imei_line->save();
                    }
                }
            }
    ...
    public function createOrUpdatePurchaseLinesOld($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
     ...
     * @return object
     */
    public function filterProductOld($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        // $result=$query->orderBy('VLD.qty_available', 'desc')
        //                 ->get();
    ...
    {
        $data = Variation::where("sub_sku",$search_term)->get();
        if(ProductImei::where('imei_number', $search_term)->exists()){     
            $products2 = Product::join('variations', 'products.id', '=', 'variations.product_id')
            ->join('product_imeis', 'product_imeis.product_id', '=', 'products.id' )
            ->leftjoin(
                'variation_location_details AS VLD',
                function ($join) use ($location_id) {
           ...
           ->select(
                ...
                'variations.sell_price_inc_tax as selling_price',
                'variations.sub_sku',
                'product_imeis.imei_number as imei_number',
                'product_imeis.id as imei_id'
            )
            ->where(["product_imeis.imei_number" => $search_term])->where('product_imeis.sold_out', 0)
            ->get();
            return  $products2;
        }
        else if(Variation::where(["sub_sku"=>$search_term])->exists()){
           ...
           ->select(
            ...
            ->where(["variations.sub_sku" => $search_term])
            ->get();
            return $products2;


        }
        else if(Product::where(["sku"=>$search_term])->exists()) {
            $products2 = Product::join('variations', 'products.id', '=', 'variations.product_id')
            ->leftjoin(
                'variation_location_details AS VLD',
                function ($join) use ($location_id) {
           ...
           ->select(
            ...
            ->where(["products.sku" => $search_term])
            ->get();
        ...
        // $query = Product::join('variations', 'products.id', '=', 'variations.product_id')
        //         ->active()
        //         ->join('product_imeis', 'products.id', '=', 'product_imeis.product_id')
        //         ->whereNull('variations.deleted_at')
        //         ->leftjoin('units as U', 'products.unit_id', '=', 'U.id')
        //         ->leftjoin(
        //             'variation_location_details AS VLD',
