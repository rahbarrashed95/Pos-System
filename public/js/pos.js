...
//sell with Imei
$('table#pos_table tbody').on('click', '.imei_row', function(){
	var imeirow = $(this);
	row_count = imeirow.find('input').val();
	localStorage.setItem("rowCount", row_count);

});

var selectedValues = [];
$(document).on('keydown', '.imei_search', function() {
    $('.imei_search').autocomplete({
        source: function (request, response) {
            $.getJSON(
                "/pos/search_imei",
                {
                    product_id: $(`.product_id${localStorage.rowCount}`).val(),
                    term: $('.imei_search').val(),
                    row_count: localStorage.rowCount,
                },
                response
            );
        },
        ...
        select: function (event, ui) {
			if (ui.item) {
                ...
                if (selectedValues.indexOf(selectedValue) !== -1) {
                    alert("Imei has already been selected.");
                    return false; // Prevent the selection
                }

                selectedValues.push(selectedValue); // Add the selected value to the list
                $(this).val(''); // Clear the input field

                add_imei_row(ui.item.value, ui.item.label, ui.item.row_count, ui.item.purchase_line_id);
            } else {
                alert(LANG.out_of_stock);
            }
        },
...
function removeValueFromArray(value) {
    ...
    if (index !== -1) {
        selectedValues.splice(index, 1); // Remove the value at the specified index
    }
}


function add_imei_row(value, label, row_count, purchase_line_id){
	var row = $('.row_count_'+row_count);
	var tr = row.parents("tr");
	var unit_price_inc_tax = __read_number(tr.find('input.pos_unit_price_inc_tax'));
	var quantity = __read_number(tr.find(`.pos_quantity`));
	var parent = $(`.imei_row_${row_count}`);
	quantity = quantity+1 ; 
	var html = `
	<div class="row">
		<div class=" col-xs-10 form-group">
		<input type="text" class="form-control" name="" id="" value="${label}">
		<input type="hidden" class="form-control value" name="products[${row_count}][imeis][]" id="" value="${value}">
		</div>
	...
	parent.append(html);
	var line_total = quantity*unit_price_inc_tax;
	__write_number(tr.find('.pos_quantity'), quantity, false);
	__write_number(tr.find("input.pos_line_total"),line_total,  false, 2);
...
$(document).on('click', 'button.decrement', function(){
    var row_count = $(this).data("parent");
    var parent = $(this).parent();
	var tr = parent.parents("tr");
    parent.remove();
	var quantity = __read_number(tr.find(`.pos_quantity`));
	quantity= quantity-1;
	__write_number(tr.find(`.pos_quantity`), quantity, false, 2);
	var unit_price_inc_tax = __read_number(tr.find('input.pos_unit_price_inc_tax'));
    var line_total = quantity*unit_price_inc_tax;
	__write_number(tr.find("input.pos_line_total"),line_total,  false, 2);
	tr.find("span.pos_line_total_text").text(
	...
	pos_total_row();
	round_row_to_iraqi_dinnar(tr);
	var value = __read_number(parent.find('.value'));
