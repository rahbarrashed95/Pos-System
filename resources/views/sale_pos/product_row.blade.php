...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	<td>
		...
		<input type="text" data-min="1" 
			...
			value="0" name="products[{{$row_count}}][quantity]" data-allow-overselling="@if(empty($pos_settings['allow_overselling'])){{'false'}}@else{{'true'}}@endif" 
			@if($allow_decimal) 
				data-decimal=1 
			@else 
				data-decimal=0 
	...
	<td>
	    <input type="text" 
			class="form-control" 
			name="products[{{$row_count}}][duration]" 
			value="365">
			
	    <!--<select class="form-control" name="products[{{ $row_count }}][duration_type]">-->
	...
	<td>
		<span data_parent="{{ $row_count }}" class="text-link text-info imei_row  cursor-pointer" data-toggle="modal" data-target="#imei_modal{{ $row_count}}">
			SN Number
			&nbsp;<i class="fa fa-info-circle"></i>
			<input type="hidden" value="{{ $row_count }}">
		</span>
		<div class="modal fade" id="imei_modal{{ $row_count}}" role="dialog">
			@include('sale_pos.imei_modal')
