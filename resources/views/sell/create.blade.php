...
@section('javascript')
    ...
    <script type="text/javascript">
    	...
    	function generateImeiInputs(element, rowCount) {
            let quantity = parseInt(element.value);
            let imeiRow = document.getElementById('imei_row_' + rowCount);
            imeiRow.innerHTML = '';
            ...
            for (let i = 0; i < quantity; i++) {
                ...
                inputGroup.innerHTML = `
                    <div class="col-xs-10 form-group">
                        <input type="text" class="form-control ime_input" data-id="${rowCount}" name="purchases[${rowCount}][imei][${i}][imei_number]">
                    </div>
                    ${i === quantity - 1 ? '<button type="button" class="btn btn-success increment button_${rowCount}" data-parent="${rowCount}" onclick="addImeiInput(${rowCount})">+</button>' : ''}
                `;
                imeiRow.appendChild(inputGroup);
            }
        }
        
        function addImeiInput(rowCount) {
            let imeiRow = document.getElementById('imei_row_' + rowCount);
            let childCount = imeiRow.children.length;
    
            let inputGroup = document.createElement('div');
            inputGroup.className = 'row';
            inputGroup.innerHTML = `
                <div class="col-xs-10 form-group">
                    <input type="text" class="form-control ime_input" data-id="${rowCount}" name="purchases[${rowCount}][imei][${childCount}][imei_number]">
                </div>
                <button type="button" class="btn btn-success increment button_${rowCount}" data-parent="${rowCount}" onclick="addImeiInput(${rowCount})">+</button>
            `;
            imeiRow.appendChild(inputGroup);
        }
